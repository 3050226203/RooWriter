...
export class Task {
	...
	> {
		...
		// [RooWriter] Optimization: Filter out intermediate failed attempts and tool errors
		// This reduces token usage by removing "noise" from the context history
		// We keep the last few messages intact to maintain immediate context
		const optimizationThresholdIndex = Math.max(0, messages.length - 6) // Keep last 6 messages full fidelity

		for (let i = 0; i < messages.length; i++) {
			const msg = messages[i]

			// [RooWriter] Optimization logic:
			// If we are before the threshold (older history), skip tool results that were errors
			// or tool uses that led to errors, unless they are critical for context.
			if (i < optimizationThresholdIndex) {
				// If it's a tool result indicating a failure or restriction, we might want to skip it
				// provided the next message isn't a direct correction of it.
				// (Simplified implementation: Just passing through for now, but placeholder for logic)
				// Future enhancement: Check if msg is a tool_result with isError=true
			}
