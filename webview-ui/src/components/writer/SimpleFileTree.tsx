...
const MOCK_FILES = [
	...
	{ name: "Characters", path: "characters", type: "folder", children: [
...
const FileNodeItem: React.FC<{ node: FileNode; level: number }> = ({ node, level }) => {
	...
	return (
		<div className="flex flex-col">
			...
			>
				<span className={`codicon codicon-${node.type === "folder" ? "folder" : "file"} mr-2 opacity-70`}></span>
...
export const SimpleFileTree: React.FC = () => {
	return (
		<div className="flex flex-col h-full overflow-y-auto border-t border-[var(--vscode-widget-border)] pt-2">
			<div className="px-4 pb-2 text-xs font-bold opacity-50 uppercase tracking-wider">
				Project Files
			</div>
			<div className="flex flex-col">
				{MOCK_FILES.map((node) => (
